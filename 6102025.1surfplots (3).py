...
def make_rose_plot(df,
    ...
    ax1.bar(rose_df.WD, rose_df[obsvar], normed=True, cmap=colors, label = "Observed "+ obsvar)

    # draw ax2
    ax2 = WindroseAxes.from_ax(fig = fig, rect=rect_set2)
    ax2.bar(rose_df.winddir, rose_df[modvar], normed=True, cmap=colors, label = "Modeled "+ modvar)
    ...
    ax1.set_xlabel(f"Observed {obsvar}", fontsize=text_kwargs["fontsize"]*0.9)
    ax2.set_xlabel(f"Modeled {modvar}", fontsize=text_kwargs["fontsize"]*0.9)
