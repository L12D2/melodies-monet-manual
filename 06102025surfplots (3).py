...
def make_rose_plot(pairdf,
    ...
    rose_obs = pairdf.copy()
    rose_mod = pairdf.copy()
    print(len(pairdf))
    #print(pairdf_test)

    df_obs = rose_obs.copy() #pairdf(subset=["WD", "WS"], axis = 0)
    df_mod = rose_mod.copy() #pairdf(subset=["winddir", "windspeed"], axis = 0)

    # this line does something odd. I think the issue just goes back to where pairdf is at. 
    # Examples of what the numbers should look like in jupyter hub 
    
    df_obs = df_obs.dropna(subset=["WD", "WS"], axis = 0)
    df_mod = df_mod.dropna(subset=["winddir", "windspeed"], axis = 0) 
    #need to generalize this to ensure that windspeed can be changed out. 

    print(len(df_obs))
    print(len(df_mod))

    # if epa_region is not None: 
    #     region = comb_bx[comb_bx["epa_region"]=="R9"] # need to make sure this is the yaml option 
    #     df_obs = region.copy()
    #     df_mod = region.copy()
    ...
    ax1.bar(rose_obs.WD, rose_obs.WS, normed=True, cmap=colors, label = "Windspeed")

    # draw ax2
    ax2 = WindroseAxes.from_ax(fig = fig, rect=rect_set2)
    ax2.bar(rose_mod.winddir, rose_mod.windspeed, normed=True, cmap=colors, label = "Windspeed")
    ...
    legend_title = "Wind Speed (m/s)" # dynamically set eventually 
