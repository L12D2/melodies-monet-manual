    ...
    def plotting(self):
        ...
        for grp, grp_dict in plot_dict.items():
            ...
            if plot_type == "rose_plot":
              ...
              #  region_list = grp_dict['region_list']
              #  region_name = grp_dict['region_name']
            # add settings for epa_region and msa_name
        ...
        if plot_type.lower() == 'rose_plot':
            model_label = p.model
            obs_label = p.obs

            # grab mapping for the obs labeling
            #obs_config = self.control_dict['model'][model_label]["mapping"][obs_label]                      
            # triggers the creation of the roseplot
            roseplot_vars = self.control_dict['model'][model_label]["mapping"][obs_label]#self.control_dict['model'][model_label]["variables"]
            print(roseplot_vars)

            for modvar, obsvar in roseplot_vars.items():
                pairdf = pairdf_all.reset_index().dropna(subset=["WD", "winddir", obsvar, modvar])
                # non optional. WD (obs) and winddir(model) will always be required
                # obs var and modvar can be optional arguments.
            
                df = splots.make_rose_plot(
                    pairdf,
                    obsvar=obsvar,
                    modvar=modvar,
                    #column=obsvar,
