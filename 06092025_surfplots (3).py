...
def make_rose_plot(pairdf,
    ...
    #print(comb_bx.columns)
    df_obs = pairdf.dropna(subset=["WD", "WS"], axis = 0).copy()
    df_mod = pairdf.dropna(subset=["winddir", "windspeed"], axis = 0).copy()
    #print(df_obs)
    ...
    ax1.bar(df_obs.WD, df_obs.WS, normed=True, cmap=colors, label = "Windspeed")
    
    ax2 = WindroseAxes.from_ax(fig = fig, rect=rect_set2)
    ax2.bar(df_mod.winddir, df_mod.windspeed, normed=True, cmap=colors, label = "Windspeed")

    # add pollution rose documentation eventually. 
    
    ax1.set_xlabel("Observed", fontsize=text_kwargs["fontsize"])
    ax2.set_xlabel("Modeled", fontsize=text_kwargs["fontsize"])
