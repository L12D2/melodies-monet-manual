...
def make_multi_boxplot(comb_bx, label_bx,region_bx,region_list = None, region_name=None, interval_labels=None, interval_var=None, interval_list=None, model_name_list=None,ylabel = None, vmin = None, vmax = None, outname='plot',  
    ...
    if region_list is not None:
        ...
        x_data = "Regions"
        #data_obs['Regions']=region_bx['set_regions'].values # interval plotting logic. 
        #to_concat = []
        #to_concat.append(data_obs[['Value','model','Regions']])
        #tdf =pd.concat(to_concat)
        #tdf['interval_str'] = tdf['interval'].astype(str)
        #data_plot = tdf.loc[tdf.interval_str.isin(acro)]

    #tdf['interval_str'] = tdf['interval'].astype(str)
    #tdf =pd.concat(to_concat)

    # debug
    #print("tdf['interval'].unique():", tdf['interval'].unique())
    #print("interval_labels:", interval_labels)

    # needed to convert a list of strings to a string so the data would populate
    # tdf['interval_str'] = tdf['interval'].astype(str)
    # interval_labels_str = [str(lab) for lab in interval_labels]
